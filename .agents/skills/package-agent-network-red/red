#!/usr/bin/env bun
// Agent Network launcher — a keyless, policy-gated Agent Network demo on Vultr.
//
...
//
// COLORS_PAR_PROFILE is the exception — agent-network refuses to run when it
// is set. The profile names this project's work directory, OpenTofu state key,
// machine keypair and cloud resources, and overriding it from the environment
// can only point agent-network at another project's state.
//
...
// own: validation, the graph and the steps all live in the `package-agent-network-red`
...
//
// "package-agent-network-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/agent-network#<sha>" in place.
//
...
const PINS = {
  "package-agent-network-red": "github:getcolors/agent-network#ae64a3e4429f01fd9a23019ee7e8a061d0040aef",
...
// add-deps. AGENT_NETWORK_LIB_ROOT overrides everything with a working tree: point it
// at the `red/` directory of an agent-network checkout whose dependencies are
 ...
 *
 * The payload's canonical home is skills/package-agent-network-red/ inside the
 * agent-network repository, whose working tree is the point: resolving a pinned copy
 * from the cache would quietly test the pinned commit instead of the edits
 * under test.
 */
function checkoutRedDir() {
  const candidate = join(import.meta.dir, "..", "..", "red");
  const manifest = readManifest(candidate);
  return manifest?.name === "package-agent-network-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-agent-network-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-agent-network-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let agentNetwork;
try {
  agentNetwork = await import("package-agent-network-red");
} catch (err) {
  ...
  const libRoot = process.env.AGENT_NETWORK_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    agentNetwork = await importWorkingTree(overrideDir, "AGENT_NETWORK_LIB_ROOT");
    ...
    agentNetwork = await importWorkingTree(redDir, "checkout");
  } else if (!PINS["package-agent-network-red"]) {
    console.error(
      "red: this launcher carries no agent-network pin.\n" +
        "It has not been stamped by `bb pin`, which cannot run until the " +
        "agent-network repository has been pushed.\n" +
    ...
    // not to a path.
    agentNetwork = await import(Bun.resolveSync("package-agent-network-red", dir));
  }
}

export const run = agentNetwork.run;
if (import.meta.main) await agentNetwork.exec();
